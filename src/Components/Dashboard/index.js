...
function Dashboard() {
  ...
  const handleGoClick = () => {
    if (isValidCustomerId(manualId)) {
      ...
      setShowScanner(false); // Hide scanner/input on obtaining valid ID
      setErrorMessage(""); // Clear any previous error message
    } else {
      // ID is not valid, show error message
        ...
        {!customerID && showScanner && (
          <div>
            ...
            {inputMode === "scanner" ? (
              ...
              // Wrap input field, error message, and button
              <div className="flex justify-center flex-col items-center">
                <input
                  type="text"
                  placeholder="Enter ID Here"
                  className="shadow border rounded py-2 px-3 text-gray-700 mb-2" // Add margin-bottom (mb-2) for spacing
                  value={manualId}
                  onChange={handleInputChange}
                />
                {/* Conditionally render error message below the input field */}
                {errorMessage && (
                  <div className="text-red-500 mb-2">{errorMessage}</div> // Add margin-bottom (mb-2) for spacing
                )}
